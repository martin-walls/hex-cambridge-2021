...
const getCatPics = async (count) => {
    return axios(`http://PLACEHOLDER/images?count=${count}`, {
        method: 'GET'
    })
    .then(res=> res.data);
}
...
const SignUp = ({location}) => {
    ...
    useEffect(() => {
        getCatPics(10).then(res => res.images).then(images => setCatPics(images));
    }, [])
    ...
    <>
        ...
        <div className="cat-pics-choice-wrapper">
            {catPics.map((url, index) => <CatPicChoice key={index} imgUrl={url} onClick={() => handleChoice(url)} size="300px"/>)}
        </div>
    </>
    );
}
